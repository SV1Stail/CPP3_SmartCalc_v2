#include "model.h"
namespace s21 {
QHash<QString, double> Model::main_deposit_back_pays_every_month(
    const double &deposit_sum, const double &percent, const double &srok,
    const double &nalog) {
  ...
  QHash<QString, double> ret = {
      {"field_nachislenie_percents", nachislennie_percents},
      {"field_nalog_sum", nalog_sum},
      {"field_sum_at_the_end", deposit_summ_at_the_end}};
  return ret;
}
QHash<QString, double> Model::main_deposit_kapitalizacia(
    const double &deposit_sum, const double &percent, const double &srok,
    const double &nalog) {
  ...
  QHash<QString, double> ret = {
      {"field_nachislenie_percents", nachislennie_percents_hard},
      {"field_nalog_sum", nalog_sum},
      {"field_sum_at_the_end", deposit_summ_at_the_end_hard}};
  return ret;
}

QHash<QString, double> Model::main_credit_annu(const double &credit_sum,
                                               const double &percent,
                                               const double &srok) {
  double field_everymonth_payment =
      credit_sum * (percent / 1200 * pow(percent / 1200 + 1, srok)) /
      (pow(1 + percent / 1200, srok) - 1);
  double field_pereplata = field_everymonth_payment * srok - credit_sum;
  double field_all_priece = credit_sum + field_pereplata;

  QHash<QString, double> ret = {
      {"field_everymonth_payment", field_everymonth_payment},
      {"field_pereplata", field_pereplata},
      {"field_all_priece", field_all_priece}};
  return ret;
}
QHash<QString, double> Model::main_credit_diff(const double &credit_sum,
                                               const double &percent,
                                               const double &srok) {
  ...
  double field_pereplata = 0;
  double summ_left = credit_sum;
  for (int i = 0; i < srok; i++) {
    field_pereplata += ((summ_left * percent / 100 * 31) / 365.0);
    summ_left = summ_left - (credit_sum / srok +
                             ((summ_left * percent / 100 * 31) / 365.0));
  }
  double field_all_priece = credit_sum + field_pereplata;
  QHash<QString, double> ret = {
      {"every_month_payment_diff_min", every_month_payment_diff_min},
      {"every_month_payment_diff_max", every_month_payment_diff_max},
      {"field_pereplata", field_pereplata},
      {"field_all_priece", field_all_priece}};
  return ret;
}
